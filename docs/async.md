...
| `tracking/tracker.py` | **extend-with-sibling** | Orchestrates dedup → queue → payload → delivery and exposes `flush` / `flush_timeout` / `flush_atexit`. The dedup and payload stages are pure and reused; only the delivery call (which routes through the sync `Transport`) becomes async. An `AsyncTracker` sibling reuses the same queue and payload logic, awaiting an `AsyncTransport` for delivery; the sync `Tracker` is not rewritten. |
| `tracking/flush.py` | **extend-with-sibling** | `setup_periodic_flush` drives a **daemonic** `threading.Timer` (correct per coexistence rule #5 — the SDK never owns an event loop). For async callers the periodic-flush scheduler becomes an `asyncio.call_later`/task tied to the host loop (architecture's long-running-process model); the sync daemon-timer path is reused unchanged by sync callers. |
